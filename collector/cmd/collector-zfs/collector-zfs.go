...
`
	...
	app := &cli.App{
		...
		Before: func(c *cli.Context) error {
			...
			collectorZfs := "AnalogJ/scrutiny/zfs"
			...
			color.New(color.FgBlue).Fprintf(c.App.Writer, fmt.Sprintf(utils.StripIndent(
				`
			 _____ _____ _____     _____     _ _           _             
			|__   |   __|   __|   |     |___| | |___ ___| |_ ___ ___   
			|   __|   __|__   |   |   --| . | | | -_|  _|  _| . |  _|  
			|_____|__|  |_____|   |_____|___|_|_|___|___|_| |___|_|    
			%s
		...
		Commands: []*cli.Command{
			{
				...
				Action: func(c *cli.Context) error {
					...
					zfsCollector, err := collector.CreateZfsCollector(
						config,
						collectorLogger,
